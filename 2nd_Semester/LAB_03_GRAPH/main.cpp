...
void spd_sharp (graph *g, heap *h, int src, int *prev, int **sharp);
...
int main () {
    ...
    //Инициализация простого связного графа значениями
    for (int t = 0; t < g->vertexes; t++) {
        for (int k = 0; k < g->vertexes; k++) {
            if (t != k)
                graph_set_edge_weight(g, t, k, rand() % 20 + 1);
        }
    }
    ...
    heap *hp = heap_create(g->vertexes);
    int *previously = (int *)calloc((g->vertexes + 1), sizeof(int));

    spd(g, hp, 3, previously);
    ...
    graph *geralt;
    geralt = graph_create(4);
    int square_geralt = (geralt->vertexes)*(geralt->vertexes);

    int **matrix = (int**)malloc(g->vertexes*sizeof(int*));

    //Инициализация графа-решётки значениями
    for (int t = 0; t < geralt->vertexes; t++) {
        for (int k = t; k < geralt->vertexes; k++) {
                graph_set_edge_weight(geralt, t, k, 1);
        }
    }

    graph_print(geralt);

    matrix = (matrix_create(geralt));
    matrix_filling(matrix, geralt);
    matrix_print(matrix, geralt->vertexes); //ВЫВОД МАТРИЦЫ СМЕЖНОСТИ

    heap *heappy_geralt = heap_create(square_geralt);
    int *gprev = (int *)calloc((square_geralt + 1), sizeof(int));

    spd_sharp(geralt, heappy_geralt, 5, gprev, matrix);
...
void matrix_filling(int **matrix_to_fill, graph *g) {
    ...
    for (int i = 0; i < size; i++) 
    {
        for (int j = 0; j < size; j++) 
        {
            ...
            {
                if (matrix_to_fill[i*size + j][i*size + (j - 1)] == 0)
                    matrix_to_fill[i*size + j][i*size + (j - 1)] = g->m[i][j - 1];
                if (matrix_to_fill[i*size + j][i*size + (j + 1)] == 0)
                    matrix_to_fill[i*size + j][i*size + (j + 1)] = g->m[i][j + 1];
                if (matrix_to_fill[i*size + j][(i - 1)*size + j] == 0)
